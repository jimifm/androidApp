...
#define _IDATA
#define _XDATA
//#define data //add by zhanghong 2014.09.25
...
typedef struct {
    ...
    BYTE VSyncWidth ;
    BYTE VBackPorch ;
    BYTE ScanMode:1 ;	//Dongzejun:指示是否是隔行/g逐行?
    BYTE VPolarity:1 ;
    BYTE HPolarity:1 ;
} VTiming ;
...
typedef enum _Video_State_Type {
    VSTATE_PwrOff = 0,	//Dongzejun:可以认为是Unplug状态
    VSTATE_SyncWait ,		//=1
    VSTATE_SWReset,		//=2		HDMI口未连接Unplug，则要进行复位状态切换.
    VSTATE_SyncChecking,	//=3 检测SCDT同步
    VSTATE_HDCPSet,
    VSTATE_HDCP_Reset,
    VSTATE_ModeDetecting, //=6
    VSTATE_VideoOn,
    VSTATE_Reserved
} Video_State_Type ;
...
#define F_MODE_EN_UDFILT (1<<6)		//Dongzejun:UP/Down filt
