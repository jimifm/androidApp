...
#include <string.h>
#include <stdlib.h>
#include "io.h"//add by zhanghong
//#include "USBI2C.h"
//#include "cat6023_pc.h"
#include "debug.h"
#include "version.h"
#else
//#include "mcu.h"
...
#define 	RXADR			0x48//edit by zhanghong for linux i2c adpter
...
#define 	RXDEV            8
#define	_1PORT_
//###cat from mcu.h end //edit by zhanghong 

//#include "typedef.h" //edit by zhanghong 
...
#define HDMI_RX_I2C_SLAVE_ADDR 0x48
...
#define REG_RX_DEVREV	0x04	//Dongzejun:器件ID

//Dongzejun:Reg0x05
#define REG_RX_RST_CTRL	0x05
#define B_CDRRST        (1<<7)			//Dongzejun:
#define B_EN_CDRAUTORST (1<<6)
#define B_EN_AUTOVDORST (1<<5)
#define B_REGRST        (1<<4)			//Dongzejun:1->Reset the chip  0->normal operating
#define B_HDCPRST       (1<<3)
#define B_AUDRST        (1<<2)			//Dongzejun:Audio reset
#define B_VDORST        (1<<1)
#define B_SWRST         (1<<0)				//Dongzejun:软件复位

//Dongzejun:P2 Reg0x06
...
#define B_PORT_SEL_A      (0<<4)		//Dongzejun:HDMI A

#define B_PORT_MASK     (1<<7)
#define B_PWD_AFEALL	(1<<3)			//Dongzejun:Reg0x07[3:2]=11 Turn off the termination
#define B_PWDC_ETC	(1<<2)
#define B_PWDC_SRV	(1<<1)
#define B_EN_AUTOPWD	(1<<0)

//Dongzejun:Reg0x0B
#define REG_RX_VIO_CTRL	0x08	//Dongzejun:0x08
...
#define B_SCDT (1<<2)				//Dongzejun:=1 Sync Detected
#define B_VCLK_DET (1<<1)		//Dongzejun:=1 VCLK Detected
...
#define REG_RX_HDCP_CTRL	0x11
    ...
    #define B_HDCP_EN     (1<<0)		//Dongzejun:HDCP使能

#define REG_RX_HDCP_STATUS	0x12
#define B_R0_READY	(1<<6)

//Dongzejun:Reg0x13 中断1 P9
#define REG_RX_INTERRUPT1	0x13
//Dongzejun:Reg0x16
#define REG_RX_INTERRUPT_MASK1	0x16
#define B_VIDMODE_CHG   (1<<5)	// 1:Video mode更改
#define B_HDMIMODE_CHG  (1<<4) // 1:HDMI、DVI模式切换
#define B_SCDTOFF       (1<<3)			//Dongzejun:1 SCDT未检测到
#define B_SCDTON        (1<<2)			// 1:已检测到SCDT
#define B_PWR5VOFF      (1<<1)			// 1:PORTA 5V电源关闭。0:PORTA 5V电源正常
#define B_PWR5VON       (1<<0)			// 1:PORTA 5V电源正常。0:PORTA 5V电源关闭

//Dongzejun:Reg0x14 Package.P-.9
#define REG_RX_INTERRUPT2	0x14
//Dongzejun:Reg0x17
#define REG_RX_INTERRUPT_MASK2	0x17
#define B_PKT_CLR_MUTE (1<<7) //Reg0x19[6]清除此中断
#define B_NEW_AUD_PKT (1<<6)
#define B_NEW_ACP_PKT (1<<5)
#define B_NEW_SPD_PKT (1<<4)
#define B_NEW_MPG_PKG (1<<3)
#define B_NEW_AVI_PKG (1<<2)		//新的AVI　帧
#define B_NO_AVI_RCV (1<<1)
#define B_PKT_SET_MUTE (1<<0)    //Reg0x19[7]清除此中断


//Dongzejun:Reg0x8B P9
#define REG_RX_INTERRUPT4 0x8B
#define B_RXCK_CHG      (1<<7)
#define B_RXCKON_DET      (1<<6)	//Dongzejun:P9 检测到Clock ON中断
#define B_HDCP_OFF      (1<<5)

//Dongzejun:Reg0x8c P9
...
//Dongzejun:Reg0x15  P16
#define REG_RX_INTERRUPT3	0x15
#define REG_RX_INTERRUPT_MASK3	0x18
#define B_AUTOAUDMUTE (1<<4)
#define B_AUDFIFOERR (1<<3)
#define B_ECCERR (1<<2)
#define B_R_AUTH_DONE (1<<1)		//Dongzejun:HDCP认证完成
#define B_R_AUTH_START (1<<0)		//Dongzejun:HDCP认证开始

//Dongzejun:P10 Reg0x19
#define REG_RX_INTERRUPT_CTRL	0x19
#define	B_CLR_MODE_INT	(1<<0)		//Dongzejun:将清除这些中断信号Reg0x13 or Reg0x8B[7:6]
#define	B_CLR_PKT_INT	(1<<1)
#define	B_CLR_ECC_INT	(1<<2)
#define	B_CLR_AUDIO_INT	(1<<3)
#define	B_INTPOL	    (1<<4)				//Dongzejun:中断极性 1:低电平中断；0:高电平中断
...
//Dongzejun:Reg0x3d P7
#define REG_RX_PG_CTRL2	0x3D
#define O_OUTPUT_COLOR_MODE 6		//Dongzejun:色彩空间定义
...
#define B_PX_VIDEOSTABLE (1<<3)  //Dongzejun:指示VIDEO　是否稳定?
#define B_VIDFIELD       (1<<2)
#define B_INTERLACE      (1<<1)		//Dongzejun:指示是否是隔行/g逐行?
...
//Dongzejun:Reg0x75 P11
#define REG_RX_I2S_CTRL	0x75	//Reg0x75 I2S控制器
...
#define B_TRI_ALL  (B_TRI_VIDEOIO|B_TRI_VIDEO|B_TRI_SPDIF|B_TRI_I2S3|B_TRI_I2S2|B_TRI_I2S1|B_TRI_I2S0) //Dongzejun:=0x7f
#define B_TRI_AUDIO  (B_TRI_SPDIF|B_TRI_I2S3|B_TRI_I2S2|B_TRI_I2S1|B_TRI_I2S0)	//Dongzejun:=0x1f
#define B_TRI_MASK  ~(B_TRI_VIDEOIO|B_TRI_VIDEO|B_TRI_SPDIF|B_TRI_I2S3|B_TRI_I2S2|B_TRI_I2S1|B_TRI_I2S0)  //Dongzejun:= ~0x7f
...
//Dongzejun:状态寄存器Reg0x92
#define REG_RX_RDROM_STATUS	0x92
#define B_ROMIDLE   		0x01		//Dongzejun:ROM 空闲
...
#define FLAG_CLEAR_INT_MODE 	0x01
#define FLAG_CLEAR_INT_PKG 	0x02
#define FLAG_CLEAR_INT_ECC 	0x04
#define FLAG_CLEAR_INT_AUDIO 0x08
#define FLAG_CLEAR_INT_MUTECLR 0x40
#define FLAG_CLEAR_INT_MUTESET 0x80
...
//Dongzejun:(0x80|0x40|0x08|0x04|0x02|0x01)=0xCF

#define FLAG_CLEAR_INT_MASK (~FLAG_CLEAR_INT_ALL)  //Dongzejun:=0x30
...
typedef struct {
    BYTE AudioFlag ;			//Dongzejun:reg8a[7:4] 
    BYTE AudSrcEnable ;	//Dongzejun:reg8a[3:0] Audio SCR
    BYTE SampleFreq ;		//Dongzejun:reg84[3:0] HDMI输入采样频率值
    BYTE ChStat[5] ;
} AUDIO_CAPS ;

#define Switch_HDMIRX_Bank(x)   HDMIRX_WriteI2C_Byte(0x0f,(x)&1)

#define CAT_HDMI_PORTA 0		//Dongzejun:HDMI　端口A
#define CAT_HDMI_PORTB 1		//Dongzejun:HDMI　端口B
...
BYTE getCAT6023xCnt() ;
void getCAT6023Regs(BYTE *pData) ;
BYTE getCAT6023OutputColorMode() ;
BOOL getCAT6023AudioInfo(BYTE *pbAudioSampleFreq, BYTE *pbValidCh) ;
BYTE getCAT6023OutputColorDepth() ;
...
void SetCAT6023VideoOutputFormat(BYTE bOutputMapping, BYTE bOutputType, BYTE bOutputColorMode) ;

BOOL  GetACPPacket(BYTE *pData) ;
BOOL  GetVENDORSPECInfoFrame(BYTE *pData) ;
BOOL  GetMPEGInfoFrame(BYTE *pData) ;
BOOL  GetAudioInfoFrame(BYTE *pData) ;
BOOL  GetAVIInfoFrame(BYTE *pData) ;

BYTE getCAT6023AudioStatus();
BOOL getCAT6023AudioChannelStatus(BYTE ucIEC60958ChStat[]);
void setCAT6023_HBROutput(BOOL HBR_SPDIF);
void setCAT6023_SPDIFOutput();
void setCAT6023_I2SOutput(BYTE src_enable) ;
